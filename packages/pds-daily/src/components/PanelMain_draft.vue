...
const auth2ClientIDTextField = ref(null)
// defineExpose({
//   auth2ClientIDTextField
// })
const auth2ClientID = ref('')
const auth2RedirectURI = ref('http://localhost:5173/')
const rules = {
  required: (value: string) => {
    console.log('debug rule.required: ', value)
    return false
  }
}
...
const authorizeRequest = apis.Auth.authorize()
...
const handleAuthorizeWithCredentials = async () => {
  try {
    const validation = await auth2ClientIDTextField.value.validate()
    console.log({ validation })
  } catch (error) { console.log('error validating: ', error) }
  // const client_id = auth2ClientID.value
  // const redirect_uri = auth2RedirectURI.value
  // const params = { client_id, redirect_uri }
  // const request = apis.Auth.authorizeWithParams({ params })
  // const authorizeParams = new URLSearchParams(request.params)
  // const authorizeLink = `${request.url}/?${authorizeParams.toString()}`
  // window.location.href = authorizeLink
}
</script>
...
<template>
  <div class="panel-main">
    <div class="panel-main__content">
      ...
      <v-chip-group class="task-item-detail-filters" v-model="taskItemDetailFitlers" multiple
      ...
      <GridLayout v-model:layout="layoutFromTasksList" :col-num="COL_NUM" :class="[gridLayoutClasses.contentEmpty]"
        :responsive="false" :is-bounded="true" :use-css-transforms="true" :vertical-compact="false"
        @layout-updated="layoutUpdatedEvent">
        <!-- <Popover v-for="item in layoutFromTasksList" :item="item" :key="item.i"> -->
        <GridItem v-for="item in layoutFromTasksList" :item="item" :key="item.i" :x="item.x" :y="item.y" :w="item.w"
          :h="item.h" :i="item.i" @mouseover="onItemHover($event, item)">
    ...
    <div class="panel-main__content">
      <v-btn @click="handleAuthorizeWithCredentials">authorize #2</v-btn>
      <v-text-field label="Client ID" ref="auth2ClientIDTextField" v-model="auth2ClientID" :rules="[rules.required]">
        <template #append="ctx">
          {{ ctx }}
        </template>
      </v-text-field>
      <v-text-field v-model="auth2RedirectURI" label="Redirect URI" disabled readonly
        class="disabled-input"></v-text-field>
    </div>
  </div>
</template>
